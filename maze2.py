...
os.chdir(r"C:\Users\Dell\Projects\GitExercise_TC4L")
...
def check_for_collision():
    if player.distance(monster) < 24:  # Check if player is near the monster
        turtle.bye()  
        open_combat_window(player.fire_power, player.life, player.shield, player.fire_power) 

# Function to open the combat window
def open_combat_window(player_fire_power, player_life, player_shield, max_bullets):
    pygame.init()
    ...
    player_shield = player_shield  # Number of shields the player has
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and max_bullets_shot < player_fire_power:
        ...
        if enemy_rect.colliderect(player_rect):
            ...
            if player_shield > 0:
                player_shield -= 1
                print(f"Shield used! Remaining shields: {player_shield}")
            else:
        ...
        for enemy_bullet in enemy_bullets:
            enemy_bullet.x -= 10
            if enemy_bullet.colliderect(pygame.Rect(player_rect.x, player_rect.y-50, player_rect.width, player_rect.height)):
                if player_shield >  0:
                    player_shield -= 1
                    print(f"Shield used! Remaining shields: {player_shield}")
